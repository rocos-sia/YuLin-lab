...
class Myaction:
    def __init__(self):
        ...
        self.server = actionlib.SimpleActionServer(
            "Get_sample", AddintsAction, self.cb, False
        )
        self.server.start()
    ...
    def cb(self, goal):
        ...
        try:
            ...
            if goal_num == 1:
                ...
                plat_sample = ttClient.recv(BUFFSIZE).decode()
                if plat_sample != "00":
            ...
            logger.error(error)
            logger.error(
                "task!failed 03!+流程操作失败，流程为%s", self.action_command[record[-2]]
            )
...
class Myaction1:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Centrifuge", AddintsAction, self.cb, False
        )
        self.server.start()
        self.cen = centrifuge()
        self.action_command = [
...
class Myaction2:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Seperation_acid", AddintsAction, self.cb, False
        )
        self.server.start()
        self.action_command = ["小车移动到废液池位置", "二维码矫正", "横向夹取试管进行倾倒", "机械臂恢复到设置状态"]
        self.sep = seperation(0)
        # rospy.loginfo("服务器启动...")
    ...
    def cb(self, goal):
        ...
        try:
            ...
            if goal_num == 1:
                ...
                self.sep.throw_tube(param, model_flag)
...
class Myaction2_1:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Seperation_alkali", AddintsAction, self.cb, False
        )
        self.server.start()
        self.action_command = ["小车移动到废液池位置", "二维码矫正", "横向夹取试管进行倾倒", "机械臂恢复到设置状态"]
        self.sep = seperation(1)
        # rospy.loginfo("服务器启动...")
...
class Myaction3:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Drying", AddintsAction, self.cb, False
        )
        self.server.start()
        self.dry = drying()
        self.action_command = [
...
class Myaction4:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Recharge", AddintsAction, self.cb, False
        )
        self.server.start()
...
class Myaction5:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Put_sample", AddintsAction, self.cb, False
        )
        self.server.start()
    ...
    def cb(self, goal):
        ...
        try:
            ...
            if goal_num == 1:
                rospy.loginfo("小车移动到取试管位置")
                duco_cobot.movej(midpose, 30, 20, 0, True)
                data = "11"
                ttClient.send(data.encode())
                plat = ttClient.recv(BUFFSIZE).decode()
                if plat != "00":
...
class Myaction7:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "Poweroff", AddintsAction, self.cb, False
        )
        self.server.start()
...
class Myaction8:
    def __init__(self):
        # SimpleActionServer(topic->name, ActionSpec, execute_cb=None, auto_start=True)
        self.server = actionlib.SimpleActionServer(
            "model", AddintsAction, self.cb, False
        )
        self.server.start()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 设置 写入日志文件的Handler 的日志格式
        fh.setFormatter(format)
        fh_debug.setFormatter(format)
        # 将Handler添加至日志记录器logger里
