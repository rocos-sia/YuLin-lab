...
class aruco:
    ...
    def __init__(self):
        ...
        # 开门判定
        self.open_door = 1  # 1 代表打开 0代表未打开
    ...
    def mark_callback(self, data):
        ...
        self.pose_flag.orientation.x = (self.pose_flag.orientation.x +
                                        data.pose.orientation.x)
        self.pose_flag.orientation.y = (self.pose_flag.orientation.y +
                                        data.pose.orientation.y)
        self.pose_flag.orientation.z = (self.pose_flag.orientation.z +
                                        data.pose.orientation.z)
        self.pose_flag.orientation.w = (self.pose_flag.orientation.w +
        ...
        if self.mark_flag == self.div:
            # mark旋转平移矩阵
            data_t = T.quat2T(
                self.pose_flag.position.x / self.div,
                self.pose_flag.position.y / self.div,
                self.pose_flag.position.z / self.div,
                self.pose_flag.orientation.x / self.div,
                self.pose_flag.orientation.y / self.div,
                self.pose_flag.orientation.z / self.div,
            ...
            # 计算机械臂移动距离
            eelink2baselink_2 = np.dot(
                np.dot(
                    np.dot(np.dot(eelink2baselink, cammer2eelink), data_t),
                    np.linalg.inv(mark2cammer),
                ),
                np.linalg.inv(cammer2eelink),
            )
    ...
    def mark(self, aruco_id, div, sum):
    ...
    def test_callback(self, data):
        self.pose_num.orientation.x = (self.pose_num.orientation.x +
                                       data.pose.orientation.x)
        self.pose_num.orientation.y = (self.pose_num.orientation.y +
                                       data.pose.orientation.y)
        self.pose_num.orientation.z = (self.pose_num.orientation.z +
                                       data.pose.orientation.z)
        self.pose_num.orientation.w = (self.pose_num.orientation.w +
                                       data.pose.orientation.w)
        self.mark_num = self.mark_num + 1

        if self.mark_num == self.num:
            # mark旋转平移矩阵
            mark2camera = T.quat2euler([
                self.pose_num.orientation.x / self.num,
                self.pose_num.orientation.y / self.num,
                self.pose_num.orientation.z / self.num,
                self.pose_num.orientation.w / self.num,
            ])
    ...
    def test_key_open(self, aruco_id):
        try:
            print("recognize aruco")
            # 识别固定次数
            print("start")
